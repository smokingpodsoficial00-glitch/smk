...
async function testFetch() {
  ...
  if (activeData) {
    console.log("Produtos ativos encontrados:", activeData.map(p => `${p.brand} ${p.name} - ${p.flavor} (is_active: ${p.is_active})`));
  }
  ...
  if (allData) {
    console.log("Todos os produtos:", allData.map(p => `${p.brand} ${p.name} - ${p.flavor} (is_active: ${p.is_active})`));
  }
}
